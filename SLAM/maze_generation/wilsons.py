...
def generate_wilsons_maze(n, path_symbol=" ", wall_symbol="x"):
    ...
    # if this 1 isnt added here
    # then on the last iteration(s), although the
    # algorithm knows theres a spot it hasnt covered
    # when it assigns that value to be 1 (to then be 2) there technically
    # are no more zeroes so the program terminates
    # leaving an empty hole
    while (0 in _been_visited) or (1 in _been_visited):
        ...
        _been_visited[_q*(_cury + _dy) + _curx + _dx] = max(_been_visited[_q*(_cury + _dy) + _curx + _dx], 1)
        _cury += _dy
        _curx += _dx
